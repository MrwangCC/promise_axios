...
(function (window) {
    ...
    class Promise {
        ...
        constructor(excutor) {
            ...
            self.status = PENDING; // 指定状态属性，初始值为pending，代表初始未确定的状态
            ...
            } catch (error) { // 执行器出错，当前promise变为失败
                reject(error);
            }
        }
        ...
        /**
         * 用来指定成功/失败回调函数的方法
         *     1. 如果当前promise是pending状态，保存回调函数
         *     2. 如果当前promise是resolved状态，异步执行成功的回调函数onResolved
         *     3. 如果当前promise是rejected状态，异步执行失败的回调函数onRejected
         * 返回一个新的Promise对象
         *     它的结果状态由onResolved或者onRejected执行的结果决定
         *     2.1. 抛出error ==> 变为rejected，结果值为error
         *     2.2. 返回值不是promise ==> 变为resolved，结果值为返回值
         *     2.3. 返回值为promise ==> 由这个promise的决定新的promise的结果(成功/失败)
         */
                ...
                function handle(callback) {
                    try {
                        ...
                        if (!(result instanceof Promise)) { // 2.2. 返回值不是promise ==> 变为resolved，结果值为返回值
                            resolve(result);
                        } else { // 2.3. 返回值为promise ==> 由这个promise的决定新的promise的结果(成功/失败)
                            result.then(
                                value => resolve(value),
